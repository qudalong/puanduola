<template>
  <div class="content">
    ...
    <div class="txt">
      游戏规则：每次抽奖需消耗1PDR，根据运气会获得1PDR，2PDR，5PDR，10PDR或谢谢参与，每日限抽奖10次，祝你好运！
    </div>
                ...
                that.timer = setInterval(()=>{
                  that.$post('/apiv1/index/goLettery').then(res=>{
                    ...
                    if(res.code == 1){
                      that.offOn = true;
                      layer.open({
                        content: res.data == 0 ? '谢谢参与' : '恭喜您获得' + res.data + 'PDR',
                        btn: '确定'
                      });
                      that.getlist()
                    }else{
